...
import { createMagazineColumns } from '@/components/modules/magazine/columns';
...
const Magazine = () => {
  ...
  const [releaseId, setReleaseId] = useState<string | number | undefined>(
    (router?.query?.releaseId as string) || undefined
  );
  const [countryCode, setCountryCode] = useState<string | undefined>(
    (router?.query?.countryCode as string) || undefined
  );

  // React Query: cache automático por queryKey ['magazines', page, name, sortBy, sortDirection, ...]
  const { data = {}, isLoading } = useMagazines({
    page,
    name,
    sortBy,
    sortDirection,
  ...
  const columns = React.useMemo(() => createMagazineColumns(), []);
  ...
  const updateURL = React.useCallback(
    ...
    [page, name, sortBy, sortDirection, typeId, releaseId, countryCode, router]
  );
  ...
  return (
    <>
      <Head>
        <title>Coanime.net - Revistas</title>
      </Head>
      <AppLayout>
                  ...
                  <>
                    ...
                    <FilterSelect
                      ...
                      onChange={(value) => {
                        setReleaseId(value);
                        setPage(1);
                        updateURL({ releaseId: value, page: 1 });
                      }}
