 ...
 */
export function DataTableRowsSkeleton({ rows = 10, columns = 6 }: DataTableSkeletonProps) {
  return (
    <>
      {/* Skeleton rows */}
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <tr key={rowIndex} className="border-b border-gray-100 transition-colors hover:bg-gray-50 data-[state=selected]:bg-gray-100">
