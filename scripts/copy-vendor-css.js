...
const vendorCss = [
  ...
  {
    from: path.join(__dirname, '../node_modules/suneditor/dist/css/suneditor.min.css'),
  ...
  {
    from: path.join(__dirname, '../node_modules/swiper/modules/navigation/navigation.min.css'),
    to: path.join(__dirname, '../src/styles/vendor/swiper-navigation.css'),
  },
  {
    from: path.join(__dirname, '../node_modules/swiper/modules/pagination/pagination.min.css'),
    to: path.join(__dirname, '../src/styles/vendor/swiper-pagination.css'),
  },
  {
    from: path.join(__dirname, '../node_modules/swiper/modules/scrollbar/scrollbar.min.css'),
...
vendorCss.forEach(({ from, to }) => {
  ...
  if (sourceFile) {
    ...
    console.log(`✓ Copiado: ${path.basename(sourceFile)} → ${path.relative(process.cwd(), to)}`);
  } else {
    const filesToCheck = Array.isArray(from) ? from : [from];
    console.warn(`⚠ No encontrado: ${filesToCheck.map((f) => path.basename(f)).join(' o ')}`);
    console.warn(`  Intentó buscar en: ${filesToCheck[0]}`);
    
    // Crear un archivo vacío para evitar errores de módulo no encontrado
    fs.writeFileSync(to, '/* Archivo CSS de vendor - copiar manualmente desde node_modules */\n');
    console.warn(`  ⚠ Creado archivo vacío: ${path.relative(process.cwd(), to)}`);
  }
});
